...
export default function CommentComposer({ postId, authorId, onAdded }:{
  ...
  async function submit(){
    ...
    setBusy(true)
    const { error } = await supabase.from('comments').insert({ post_id: postId, author: authorId, content: text })
  ...
  return (
    <div style={{display:'flex', gap:8}}>
      ...
      <button onClick={submit} style={{background:'#8B7E6A', color:'#fff', padding:'8px 12px', borderRadius:12}}>Send</button>
    </div>
  )
}
